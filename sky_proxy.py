...
class CacheStream:
    ...
    def invalidar(self, chave: str | None = None):
        with self._lock:
            if chave:
    ...
    def proximos_a_expirar(self, segundos: int = TOKEN_REFRESH_AHEAD) -> list[str]:
        agora = time.time()
        with self._lock:
            return [
...
def capturar_url_stream(channel_key: str) -> dict | None:
    """
...
def _buscar_mpd(manifest_url: str, req_headers: dict) -> str:
...
def _filtrar_mpd(mpd_str: str, qualidade: str) -> str:
    """
...
def _obter_dados_canal(channel_key: str) -> dict | None:
...
class ProxyHandler(BaseHTTPRequestHandler):
    ...
    def _resposta(self, codigo: int, content_type: str, corpo: bytes | str):
